...
function findBaseRef() {
  ...
  const remoteCandidates = ["refs/remotes/origin/main", "refs/remotes/origin/master"];
  ...
  const headPrev = runCommand("git", ["rev-parse", "--verify", "--quiet", "HEAD~1"]);
...
const diffOutput = captureCommand("git", ["diff", "--name-only", `${mergeBase}...HEAD`]) ?? "";
...
const vitestResult = runCommand("pnpm", ["exec", "vitest", "related", "--run", ...changedFiles], {
  stdio: "inherit",
});
